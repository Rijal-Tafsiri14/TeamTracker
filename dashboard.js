...
// 1. Siapkan Suara Lonceng (Menggunakan library suara standar Google yang ringan & aman)
const notifSound = new Audio('https://actions.google.com/sounds/v1/alarms/beep_short.ogg');
notifSound.volume = 0.5; // Atur volume (0.0 sampai 1.0)
...
if (btnNotif && notifDropdown) {
    ...
    // Flag penanda agar tidak bunyi saat web pertama kali di-refresh
    ...
    onSnapshot(qNotif, (snapshot) => {
        notifList.innerHTML = '';
        let hasNew = false;
        let shouldPlaySound = false; // Flag untuk menentukan kapan harus bunyi
        ...
        // Cek data yang baru saja benar-benar disisipkan (bukan data lama)
        ...
        // Mainkan suara jika ada notif masuk (tapi bukan saat pertama load)
        if (shouldPlaySound) {
